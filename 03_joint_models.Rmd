...
Building on the notation from Section \@ref(sa-terminology-notation), let \(y_{ij} = \{ y_{ij}(t_{ij}) \ \forall \ j = 1, \dots, n_i \}\) be the observed longitudinal response for the \(i\)^th^ subject, with \(y_{ij}(t_{ij})\) the observed response at time \(t_{ij}\) and \(n_i\) the number of longitudinal observations.
...
Historically the full joint likelihood has been maximised using the Expectation-Maximisation algorithm [@dempster_1977]; alternatively, it is possible to use general purpose optimisers to maximise the full joint likelihood via algorithms such as the Newton-Raphson algorithm. Nevertheless, significant computational challenges persist; I will discuss them further in Chapter \@ref(compch).
