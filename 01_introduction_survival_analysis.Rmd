...
c = 5
t = pmin(s, c)
d = factor(as.numeric(s <= c), levels = 0:1, labels = c("Censored", "Event"))
...
g1 = ggplot(data, aes(x = t, y = id, shape = d)) + 
...
g2 = ggplot(data, aes(x = t - entry, y = id, shape = d)) + 
  geom_vline(xintercept = 0, lty = "dashed") +
  geom_segment(aes(x = 0, xend = t - entry, y = id, yend = id), color = "grey50") +
  geom_point(size = 2) +
  scale_shape_manual(values = c(1, 4)) +
  theme_bw() +
  theme(legend.position = "bottom") +
...
sf = survfit(Surv(time = t - entry, event = as.numeric(d)) ~ 1, data = data)
g2 = with(summary(sf, times = sort(unique(c(0:5, data$t - data$entry)))),
