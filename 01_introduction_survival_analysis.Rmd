...
In this Chapter I will define survival data and its peculiarities in Section \@ref(sa-survdata) and \@ref(sa-censoring). Terminology and notation used throughout this report will be introduced in Section \@ref(sa-terminology-notation). I will introduce common non-parametric and parametric methods in survival analysis in Sections \@ref(sa-estsurv) and \@ref(sa-parsa). I will introduce the widely used semi-parametric Cox model in Section \@ref(sa-cox). Finally, I will provide a brief overview of modern, advanced statistical methods in survival analysis in Section \@ref(sa-advanced). 
...
```{r get_process_ist_data, eval = FALSE}
...
ll = function(pars, X, t, d) {
  ...
  ll = sum(d * log_hi + log_Si) # + sum(log(t))
  # + sum(log(t)) is the same adjustment that Stata does to remove the time units from log L
  return(-ll)
}
```
...
```{r starting_values, echo = TRUE}
start = c(1, 1, rep(0, ncol(X)))
names(start) = c("lambda", "p", colnames(X))
```

The value of the log-likelihood function at the starting values is `r cute(ll(start, X, ist$t, ist$d))`. Finally, I use the `BFGS` optimiser (details in @fletcher_2000) to maximise the log-likelihood:

```{r optimisation, echo = TRUE}
fit = optim(par = start, fn = ll, X = X, t = ist$t, d = ist$d, method = "BFGS", hessian = TRUE)
```

The maximum likelihood estimates are:

```{r ml_estimates, echo = TRUE}
fit$par
```

The standard errors of the maximum likelihood estimates are obtained by inverting the resulting Hessian:

```{r se_estimates, echo = TRUE}
...
res$p = 1 - pnorm(abs(res$z), 1 - 0.05 / 2)
kable(res, digits = 3)
```

Wald test for the interaction terms, using the \(\chi^2\) test statistic (as sample size is big, using the F statistic would be equivalent):

```{r wald_interaction, echo = TRUE}
# identify the interaction terms
idx = grepl(":", names(fit$par))

# compute the W statistic
W = t(fit$par[idx]) %*% solve(fit$vcov[idx, idx]) %*% fit$par[idx]

# test it against the quantiles of a chi2 distribution with sum(idx) degrees of freedom
c(W = W, p = 1 - pchisq(W, sum(idx)))
```
